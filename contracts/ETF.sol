...
contract ETF {
    ...
    // Optional: helper to fetch all ETFs (for frontend)
    function getAllETFs() external view returns (DomainETF[] memory) {
        DomainETF[] memory result = new DomainETF[](nextETFId);
        for (uint256 i = 0; i < nextETFId; i++) {
            result[i] = etfs[i];
        }
        return result;
    }
